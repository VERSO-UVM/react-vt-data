...
from app_utils.plot import bar_chart
...
def render_sex(metrics, plot_dfs, title_geo):
    ...
    sex_col1.markdown("\2")
    sex_col1.metric(
        label="Total Population", value=f"{metrics['total_population']:,.0f}"
    )
    sex_col1.metric(
        label="Sex Ratio (Males for every 100 Females)",
        value=f"{metrics['sex_ratio']:.1f}",
    )
