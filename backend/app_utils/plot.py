...
def single_column_plot(df, selected_column):
    ...
    if column_type == "object":
        ...
        bar_title = alt.TitleParams(
            f"Bar Chart of {selected_column}",
            anchor="middle",
            dy=15,
            fontSize=18,
            font="Helvetica",
        ...
        bar_chart = (
            alt.Chart(source, title=bar_title)
            .mark_bar()
            .encode(
    ...
    # If the column is numeric
        ...
        if abs(skew) > 1:
            skew_warning = (
                "NOTE: This Distribution is **Slightly Skewed**. "
        ...
        column1.metric(
            label="**Mean**",
            value=f"{var_mean:,.2f}",
            help="The *average* of the sample.",
        )
        column2.metric(
            label="**Median**",
            value=f"{var_med:,.2f}",
            help="The *middle value* of the sample.",
        )
        column3.metric(
            label="**Standard Deviation**",
            value=f"{var_std_dev:,.3f}",
        ...
        # Histogram
        hist_title = alt.TitleParams(
            f"Histogram Distribution of {selected_column}",
            anchor="start",
            dy=15,
            fontSize=18,
            font="Helvetica",
        ...
        histogram = (
            alt.Chart(source, title=hist_title)
            .mark_bar()
            .encode(
                x=alt.X(
                    f"{selected_column}:Q",
                    bin=alt.Bin(maxbins=bin_slider),
        ...
        # Density Plot
        dens_title = alt.TitleParams(
            f"Density Distribution of {selected_column}",
            anchor="start",
            dy=15,
            fontSize=18,
            font="Helvetica",
        ...
        # Boxplot
        box_title = alt.TitleParams(
            f"Boxplot Distribution of {selected_column}",
            anchor="start",
            dy=15,
            fontSize=18,
            font="Helvetica",
        ...
        boxplot = (
            alt.Chart(source, title=box_title)
            .mark_boxplot(color="dodgerblue")
            .encode(x=alt.X(f"{selected_column}:Q", title=selected_column))
            .configure_mark()
            .configure_boxplot(size=160)
    ...
    # If the column is boolean
        ...
        pie_title = alt.TitleParams(
            "Pie Chart of {selected_column}",
            anchor="middle",
            dy=15,
            fontSize=18,
            font="Helvetica",
        ...
        pie_chart = (
            alt.Chart(source, title=pie_title)
            .mark_arc()
            .encode(
                theta="count()",
                color=alt.Color(f"{selected_column}:N"),
                tooltip=[
                    alt.Tooltip(f"{selected_column}:N", title="Category"),
...
def numeric_numeric_plots(df, col1, col2):
    ...
    grp_by = st.selectbox(
        "OPTIONAL: Select a variable to summarize by",
    ...
    scatter_title = alt.TitleParams(
        f"Scatterplot of {col1} v.s. {col2}",
        anchor="start",
        dy=15,
        fontSize=15,
        font="Helvetica",
    ...
    scatterplot = (
        alt.Chart(source, title=scatter_title)
    ...
    # REGRESSION LINE
    regression_line = (
        scatterplot.transform_regression(f"{col1}", f"{col2}")
        .mark_line()
        .encode(color=alt.value("cornflowerblue"), size=alt.value(1.5))
    )

    # LOESS LINE
    loess_line = (
        scatterplot.transform_loess(f"{col1}", f"{col2}")
        .mark_line()
        .encode(color=alt.value("tomato"), size=alt.value(1.5))
    )
    ...
    resid_df = pd.DataFrame(
        {f"Predicted {col2}": y_pred, "Prediction Error": residuals}
    )

    resids_title = alt.TitleParams(
        f"Residual Plot of {col2} Predictions",
        anchor="start",
        dy=15,
        fontSize=15,
        font="Helvetica",
    ...
    resids = (
        alt.Chart(resid_df, title=resids_title)
        .mark_square(color="tomato")
        .encode(
            x=alt.X(f"Predicted {col2}", title="Predicted"),
            y=alt.Y("Prediction Error", title="Residual"),
            tooltip=[f"Predicted {col2}", "Prediction Error"],
        )
        .interactive()
    )
    ...
    df["x_bin"] = pd.cut(
        df[col1], bins=col1_bins_unique, labels=col1_labels, include_lowest=True
    )
    # Create bins for the y axis
    df["y_bin"] = pd.cut(
        df[col2], bins=col2_bins_unique, labels=col2_labels, include_lowest=True
    )
    ...
    # Altair heatmap
    heatmap_title = alt.TitleParams(
        f"Heatmap Distribution of {col1} v.s. {col2}",
        anchor="start",
        dy=15,
        fontSize=15,
        font="Helvetica",
...
def display_numeric_numeric_plots(
    df, col1, col2, scatterplot, scatterplot_with_lines, resid_plot, heatmap
):
    """
    ...
    st.subheader("Scatterplots", help="HELLO!")
    ...
    with column2:
        st.altair_chart(
            (scatterplot_with_lines).interactive(), use_container_width=True
        )
...
def numeric_categorical_plots(df, col1, col2):
    ...
    if col1_type in ["int64", "float64"]:
        ...
        # MULTI BOXPLOT
        mbox_title1 = alt.TitleParams(
            f"Boxplot Distributions of {col1} by {col2}",
            anchor="start",
            dy=15,
            fontSize=15,
            font="Helvetica",
        ...
        multi_box = (
            alt.Chart(source, title=mbox_title1)
        ...
        error_title1 = alt.TitleParams(
            f"95% Confidence Intervals of {col1} by {col2}",
            anchor="start",
            dy=15,
            fontSize=15,
            font="Helvetica",
        ...
        error_bars = (
            alt.Chart(source, title=error_title1)
    ...
    # If the first column is categorical and the second is numeric
        ...
        # MULTI BOXPLOT
        mbox_title2 = alt.TitleParams(
            f"Boxplot Distributions of {col2} by {col1}",
            anchor="start",
            dy=15,
            fontSize=15,
            font="Helvetica",
        ...
        multi_box = (
            alt.Chart(source, title=mbox_title2)
            .mark_boxplot(size=40)
            .encode(
        ...
        # CONFIDENCE INTERVALS WITH MEANS
        error_title2 = alt.TitleParams(
            f"95% Confidence Intervals of {col2} by {col1}",
            anchor="start",
            dy=15,
            fontSize=15,
            font="Helvetica",
        ...
        error_bars = (
            alt.Chart(source, title=error_title2)
            .mark_errorbar(extent="ci")
            .encode(
...
def display_numeric_categorical_plots(
    df, numeric_col, non_numeric_col, multi_box, confint_plot
):
    """
...
def categorical_categorical_plots(df, col1, col2):
    ...
    # HEATMAP
    heat_title = alt.TitleParams(
        f"Heatmap Table of {col1} and {col2}",
        anchor="start",
        dy=15,
        fontSize=15,
        font="Helvetica",
    ...
    heatmap = (
        alt.Chart(source, title=heat_title)
        .mark_rect()
        .encode(
            x=f"{col2}:O",
            y=f"{col1}:O",
    ...
    # STACKED BAR CHARTS
    stacked_title = alt.TitleParams(
        f"Stacked Bar Chart of {col1} by {col2}",
        anchor="start",
        dy=15,
        fontSize=15,
        font="Helvetica",
    ...
    stacked_bar = (
        alt.Chart(source, title=stacked_title)
        .mark_bar()
        .encode(
    ...
    stacked_100_title = alt.TitleParams(
        f"Percentage Distribution of {col2} by {col1}",
        anchor="start",
        dy=15,
        fontSize=15,
        font="Helvetica",
    ...
    stacked_bar_100_pct = (
        alt.Chart(stacked_df_100, title=stacked_100_title)
        .mark_bar()
        .encode(
...
def display_categorical_categorical_plots(
    df, col1, col2, freq_table, format_dict, heatmap, stacked_bar, stacked_bar_100_pct
):
    """
    ...
    if (df[col1].nunique() > 2 and df[col2].nunique() > 2) and (
        ...
        with column1:
            ...
            st.dataframe(
                freq_table.style.format(format_dict, na_rep="—"), hide_index=True
            )
        # Display the heatmap
        with column2:
            st.altair_chart(heatmap, use_container_width=True)
...
def two_column_plot(df, col1, col2):
    ...
    # If two NUMERIC variables are selected
    if pd.api.types.is_numeric_dtype(col1_type) and pd.api.types.is_numeric_dtype(
        col2_type
    ):
        # Define all the needed plots
        scatterplot, scatterplot_with_lines, resid_plot, heatmap = (
            numeric_numeric_plots(df, col1, col2)
        )
        # Display all plots
        display_numeric_numeric_plots(
            df, col1, col2, scatterplot, scatterplot_with_lines, resid_plot, heatmap
        )
        # Return all plots
    ...
    # If NUMERIC and CATEGORICAL variables are selected
        ...
        col1_type in ["int64", "float64"]
        and (col2_type == "object" or col2_type.name == "category")
    ) or (
        (col1_type == "object" or col1_type.name == "category")
        and col2_type in ["int64", "float64"]
    ):
        # Create the boxplot and confidence interval plots
        multi_box, confint_plot, numeric_col, non_numeric_col = (
            numeric_categorical_plots(df, col1, col2)
        )
        # Display the plots
        display_numeric_categorical_plots(
            df, numeric_col, non_numeric_col, multi_box, confint_plot
        )
        # Return the plots
    ...
    # If two CATEGORICAL variables are selected
        ...
        freq_table, format_dict, heatmap, stacked_bar, stacked_bar_100_pct = (
            categorical_categorical_plots(df, col1, col2)
        )
        # Display the plots
        display_categorical_categorical_plots(
            df,
            col1,
            col2,
...
def group_by_plot(df, num_op, num_var, grp_by):
    ...
    # BAR CHART
    grouped_chart = (
        alt.Chart(df_grouped, title=f"{num_var_name} by {grp_by}")
        .mark_bar()
        .encode(x=alt.X(f"{grp_by}:N", sort=sort), y=alt.Y(f"{num_var_name}:Q"))
    )
...
def sort_select():
    ...
    sort_option = c1.selectbox(
        label="Sort option", options=["None", "Ascending", "Descending"], index=0
    )
