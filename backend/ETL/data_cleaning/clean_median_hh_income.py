...
**Created**:
    2026-07-14
**Description**:
    Data cleaning script for median household income.
    Derived from the `RAW.economic` DuckLake table
    Run with
python -m ETL.data_cleaning.clean_median_hh_income
"""
...
def read_raw_data() -> pd.DataFrame:
    raw_df = con.execute(
        """
        SELECT year, NAME, Value AS Median_HH_Income, geo_type,
        FROM lake.RAW.economic
        WHERE Variable = 'Median Household Income'
        ORDER BY year;
        """
    ).df()
...
def replace_unavailable_data(df: pd.DataFrame):
    df["Median_HH_Income"] = df["Median_HH_Income"].replace(-666666666.0, np.nan)
...
def add_to_lake(clean_df: pd.DataFrame):
    """
    Writes the cleaned, long-format median_hh_income dataframe
    to the CLEANED schema in DuckLake.
    """
    con.execute(
        """
        CREATE OR REPLACE TABLE lake.CLEANED.median_hh_income AS
