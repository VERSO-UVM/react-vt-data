...
def build() -> None:
    ...
    # Extension needed to handle geometries
    con.execute("INSTALL spatial; LOAD spatial;")

    # zoning: table of zoning districts with geometry as WKB for DuckDB compatibility
    con.execute(f"""
        CREATE TABLE zoning AS
        SELECT * EXCLUDE (geometry),
            ST_GeomFromWKB(geometry) AS geometry
        FROM read_parquet('{_zoning("vt_zoning.parquet")}')
    """)
    print("  [15/15] zoning")
